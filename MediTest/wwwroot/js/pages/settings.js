...
<section class="modal-panel" role="dialog" aria-modal="true" aria-labelledby="deleteAccountTitle">
  ...
  async function loadLicensePreview() {
    const statusEl = document.getElementById("licensePreviewStatus");
    const actionsEl = document.getElementById("licensePreviewActions");
    try {
      ...
      statusEl.textContent = isPremium
        ? "Premium ist aktiv – voller Funktionsumfang freigeschaltet."
        : hasSubscription
          ? "Abo aktiv."
          : s.status === "trial"
            ? `Testphase aktiv – noch ${Number(s.trialDaysRemaining) || 0} Tag(e) übrig.`
            : s.baseProductPurchased
              ? "Basiskauf aktiv. Premium-Abo für vollen Zugang verfügbar."
              : `${s.plan || "Kein aktiver Plan"}`;
      const buyBtn = isPremium
        ? '<button class="primary" type="button" disabled>Premium aktiv</button>'
        : hasSubscription
          ? '<button class="primary" type="button" onclick="manageSubscriptionFromSettings()">Abo verwalten</button>'
          : s.baseProductPurchased
            ? `<button class="primary" type="button" onclick="startSubscriptionFromSettings()" ${s.checkoutConfigured ? "" : "disabled"}>Premium-Abo kaufen</button>`
            : `<a class="button primary" href="${APP_BRAND.websiteUrl}/purchase" target="_blank" rel="noopener">Meduvalo kaufen</a>`;
      actionsEl.innerHTML = buyBtn + '<a class="button" href="/pages/license.html">Details &amp; Geräte</a>';
    } catch (e) {
      if (statusEl) statusEl.textContent = "Lizenzstatus konnte nicht geladen werden.";
    }
  }
  async function startSubscriptionFromSettings() {
    const msg = document.getElementById("msg");
    try {
