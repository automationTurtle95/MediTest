    ...
    <article class="card doc-card">
      ...
      <p class="muted">Ein gekaufter Zugang kann auf bis zu ${Number(license.maxDevices) || 2} Geräten aktiviert werden.</p>
        ...
        isPremium
          ...
          : hasSubscription
            ...
            : s.status === "trial"
              ...
              : isRestricted
                ? `Tests und Lernrunden aus vorhandenen Fragenpools können weiter genutzt werden. Dokument-, Fragen-, Katalog-, Statistik-, Bearbeitungs- und KI-Funktionen werden mit dem Abo für <b>${priceLabel(s.monthlyPriceCents, s.currency)} pro Monat</b> wieder freigeschaltet.`
    ...
    <article class="card doc-card">
      ...
      <h2>${isRestricted ? "Tests & Lernmodus bleiben verfügbar" : fullAccess ? "Voller Funktionsumfang" : "Nach dem Kauf verfügbar"}</h2>
      <ul class="license-feature-list">
        <li class="${testsAvailable ? "available" : "locked"}"><strong>Tests &amp; Lernmodus aus vorhandenen Fragenpools</strong><span>${testsAvailable ? "Verfügbar" : "Nach dem Kauf verfügbar"}</span></li>
      ...
      ${isRestricted ? `<p class="muted">Nach Ablauf der Testphase können vorhandene Tests und Lernrunden mit bestehenden Fragenpools weiter genutzt werden. Alle anderen Funktionen werden mit Premium wieder freigeschaltet.</p>` : ""}
    </article>
    <article class="card doc-card">
