...
2. nur den Zielordner `dist/MediTest-5.0.2/` entfernen
3. `dotnet restore`
...
dist/MediTest-5.0.2/windows/MediTest-5.0.2-win-x64-portable.zip
dist/MediTest-5.0.2/macos/MediTest-5.0.2-macos-x64-setup.zip
dist/MediTest-5.0.2/macos/MediTest-5.0.2-macos-arm64-setup.zip
latest.json
SHA256SUMS.txt
```
...
Danach in GitHub einen Release mit dem Tag `v5.0.2` erstellen. Der Workflow veröffentlicht:
...
Mit eingerichteten Apple-Secrets heißen die beiden Mac-Dateien stattdessen `MediTest-Setup-5.0.2-macos-<arch>.pkg`.
...
```powershell
Get-ChildItem .\dist\MediTest-5.0.2 -Recurse -File |
...
```powershell
Get-FileHash -Algorithm SHA256 .\dist\MediTest-5.0.2\windows\MediTest-Setup-5.0.2-win-x64.msi
```
