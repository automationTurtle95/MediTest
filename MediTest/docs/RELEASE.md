...
2. nur den Zielordner `dist/MediTest-4.1.0/` entfernen
3. `dotnet restore`
...
```text
dist/MediTest-4.1.0/windows/MediTest-Setup-4.1.0-win-x64.msi
dist/MediTest-4.1.0/windows/MediTest-4.1.0-win-x64-portable.zip
dist/MediTest-4.1.0/macos/MediTest-4.1.0-macos-x64-setup.zip
dist/MediTest-4.1.0/macos/MediTest-4.1.0-macos-arm64-setup.zip
latest.json
SHA256SUMS.txt
```
...
Danach in GitHub einen Release mit dem Tag `v4.1.0` erstellen. Der Workflow veröffentlicht:
...
Mit eingerichteten Apple-Secrets heißen die beiden Mac-Dateien stattdessen `MediTest-Setup-4.1.0-macos-<arch>.pkg`.
...
```powershell
Get-ChildItem .\dist\MediTest-4.1.0 -Recurse -File |
...
```powershell
Get-FileHash -Algorithm SHA256 .\dist\MediTest-4.1.0\windows\MediTest-Setup-4.1.0-win-x64.msi
```
