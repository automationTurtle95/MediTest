...
Version 5.0.11 wird als Windows-MSI, Windows-Portable-ZIP und zwei signierte, von Apple notarisierte macOS-DMGs veröffentlicht.

```text
dist/MediTest-5.0.11/
```
...
```text
MediTest-Setup-5.0.11-win-x64.msi
MediTest-5.0.11-win-x64-portable.zip
MediTest-Setup-5.0.11-macos-arm64.dmg
MediTest-Setup-5.0.11-macos-x64.dmg
latest.json
SHA256SUMS.txt
```
...
```powershell
Get-ChildItem .\dist\MediTest-5.0.11 -Recurse -File |
